...
> - **플랫폼 엔지니어**: "운영은 안전한가?" Redis 캐시와 자정 배치로 성능·안정성을 담보하자.
...
| B | 자정 배치 | `DailyStatsBatchScheduler/Service` | 전일 데이터 집계 → 캐시 무효화 → 예측 훅 |
| C | 예측 트리거 | `DailyStatsPredictionService` | 외부 모델 서비스에 재학습 요청 (REST) |
| D | 장소 상세 조합 | `PlaceDetailFacadeService/Controller` | 통계 + 리뷰 데이터를 묶어 단일 응답으로 제공 |
...
### 구현 체계
- **데이터 조회**: `PlaceStatisticsRepository`가 `visits` / `daily_likes`를 네이티브 쿼리로 집계.
- **서비스**: `PlaceStatisticsService` (DB 스냅샷) + `PlaceStatisticsQueryService` (Redis Cache-aside 패턴).
- **캐시**: `PlaceStatsCacheService`가 `place:stats:{placeId}` 키로 JSON 응답 저장.
- **응답 DTO**: `PlaceStatsResponseDTO`가 `placeName`, 누적 방문/좋아요, 최근 14일 평균 특정 시간대(11~12시·12~13시) 도착 카운트만 응답에 노출하고, `generatedAt`/`cacheTtlSeconds`/`dataSource`는 `@JsonIgnore`로 숨겨 내부 로그에서만 사용.
- **테스트**: `PlaceStatisticsServiceTest`, `PlaceStatisticsQueryServiceTest`, `PlaceStatisticsControllerTest`.

### 스냅샷 개념 정리
- "통계 스냅샷"은 특정 시점의 장소 지표 값(누적 방문자·좋아요·시간대 평균 도착)을 한 번의 쿼리로 묶어 계산한 결과다.
- 스냅샷을 DTO로 만들어두면 후속 요청이나 캐싱 시 그대로 재사용할 수 있어 DB 부하를 크게 줄인다.
- `/stats` 호출마다 스냅샷을 생성하고, TTL 동안 Redis에 저장해 반복 조회를 빠르게 처리한다.
...
- Facade는 통계 캐시를 활용해 빠른 응답을 유지하면서, 리뷰 서비스에서 가져온 데이터를 기본 5개만 노출하고 `reviewLimit` 파라미터로 조절합니다.
- 응답 구조(`PlaceDetailResponseDTO`)에는 전체 리뷰 수가 포함되어 있어 프론트에서 추가 페이지 요청 여부를 쉽게 결정할 수 있습니다.

### 흐름 요약
1. 캐시 조회 → 있으면 반환.
2. 미스 → DB 스냅샷 계산 → DTO 생성 → Redis 저장 → 응답.
...
- **배치 서비스**: `DailyStatsBatchService`
  - 업서트 후 캐시 무효화 (`PlaceStatsCacheService`).
  - 배치 상태(`BatchStatus`) 저장.
  - `DailyStatsPredictionService.scheduleRetraining(...)` 호출로 예측 재학습 트리거.
...
- `application-local.yml`
  ...
  statistics:
    ...
    prediction:
      ...
      endpoint: http://localhost:8080/internal/predictions/visit
  ```
- 로컬 개발 시 Docker `redis:7-alpine` 컨테이너 활용 권장.
- **사용 목적 정리**
  - 통계 API 응답을 5분(300초) TTL로 캐싱해 `visits`, `daily_likes` 네이티브 쿼리 비용을 줄이고 응답 속도를 일정하게 유지.
  - Cache-aside 패턴: `PlaceStatisticsQueryService`가 캐시→DB 순으로 조회 후 Redis에 저장.
  - 운영 로깅에서는 `StatsDataSource`로 "DB 계산"과 "캐시 재사용"을 구분하지만 사용자 응답에는 노출하지 않음.
  - 팀장이 관리 중인 “visit 세션” Redis와는 별도 용도이며, 세션 관리는 건드리지 않음.
...
| Service | `PlaceStatisticsServiceTest`, `PlaceStatisticsQueryServiceTest`, `DailyStatsBatchServiceTest`, `DailyStatsBatchSchedulerTest`, `DailyStatsPredictionServiceTest` | Redis/예측/배치 로직 검증 |
...
| 단계 | 설명 |
| --- | --- |
| **서비스** | - 실시간 장소 통계: 누적 방문·좋아요·시간대 도착 수 제공<br>- 일일 집계 & 캐시 유지: 자정 배치로 데이터 요약 + Redis 캐시 동기화 |
...
| **API** | - `GET /api/places/{placeId}/stats` → Cache-aside(`PlaceStatisticsQueryService`)로 누적/시간대 응답, 메타데이터는 `@JsonIgnore`<br>- `GET /api/places/{placeId}/detail` → Facade(`PlaceDetailFacadeService`)로 통계 + 최신 리뷰를 단일 응답으로 제공<br>- 자정 배치: `DailyStatsBatchService` 업서트 + 캐시 무효화, `DailyStatsBatchController`로 상태 조회/수동 실행 |
| **사용자 정보** | - 즉시 확인: 누적 방문자, 누적 좋아요, 최근 14일 평균 특정 시간대 도착 인원<br>- 운영용(숨김): 응답 생성 시각, 캐시 TTL, 데이터 출처, 배치 실행/예측 훅 결과 |
...
- **목적**: 통계 API 한 번 호출로 누적 방문(`total_visitors`), 누적 좋아요(`total_likes`), 최근 14일 평균 특정 시간대 도착(`arrivals_11_12`, `arrivals_12_13`)을 계산해 `PlaceStatsSnapshot`으로 반환.
- **구현 포인트**
...
1. **통계 조회 파이프라인**
   - `PlaceStatisticsController` → `PlaceStatisticsQueryService` → `PlaceStatisticsService`
   - QueryService는 Redis 캐시(`PlaceStatsCacheService`)를 먼저 확인하고, 미스일 때 Service가 `PlaceReadRepository`로 장소를 검증한 뒤 `PlaceStatisticsRepository`에서 스냅샷을 가져옵니다.
   - 최신 값은 캐시에 저장되고, 로그에는 `dataSource`, `cacheTtlSeconds`가 기록되어 캐시 히트/미스를 모니터링합니다.

2. **자정 배치 파이프라인**
   - `DailyStatsBatchScheduler`(00:00 실행) 또는 내부 API가 `DailyStatsBatchService`를 호출.
   - BatchService는 `PlaceDailyStatsBatchRepository`로 전일 `visits/reviews/daily_likes` 데이터를 집계하고 `place_daily_stats`에 UPSERT.
   - 이후 Redis 캐시(`PlaceStatsCacheService`)를 무효화하고, `DailyStatsPredictionService`로 예측 재학습 훅을 호출.

3. **예측 서비스 연동**
   - `DailyStatsPredictionService`는 설정(`statistics.prediction.enabled/endpoint`)에 따라 외부 모델과 연동할 수 있도록 Stub 형태로 준비되어 있습니다.

전체적으로 하나의 조회 파이프라인과 한 개의 배치 파이프라인이 캐시와 리포지토리를 공유하며 동작합니다.
...
```java
// src/main/java/com/matjom/matjom/statistics/service/PlaceStatisticsService.java:37-49
...
public PlaceStatsResponseDTO fetchPlaceStats(Long placeId) {
    ...
    return PlaceStatsResponseDTO.of(placeName, snapshot, now, cacheTtlSeconds, StatsDataSource.DATABASE);
}
```
- **설명**: 요청된 장소가 실제로 존재하는지 확인한 뒤, 데이터베이스에서 방문 수·좋아요 수 등 필요한 지표를 계산해 `PlaceStatsResponseDTO`로 구성합니다.
- `ZoneId.of("Asia/Seoul")`로 언제나 한국 표준시 기준 날짜를 계산합니다.
- `StatsDataSource.DATABASE`는 이 값이 DB에서 직접 조회됐음을 나타내며, 응답에는 포함되지 않고 로그/모니터링에서만 사용합니다.

### 12.2 캐시와 DB를 오케스트레이션하는 계층 — `PlaceStatisticsQueryService`
```java
// src/main/java/com/matjom/matjom/statistics/service/PlaceStatisticsQueryService.java:19-39
public PlaceStatsResponseDTO getPlaceStats(Long placeId) {
    ...
    try {
        ...
        cacheService.put(placeId, fresh);
        ...
        if (cached.isPresent()) {
            ...
            log.warn("Failed ... fallback to cache ...", placeId, fallback.getDataSource(), fallback.getCacheTtlSeconds(), ex);
            return fallback;
        }
        throw ex;
    }
...
```
- **설명**: 우선 Redis 캐시에서 동일 요청에 대한 결과가 있는지 확인하고, 없으면 `PlaceStatisticsService`를 호출해 새 데이터를 조회합니다. 새로 조회한 데이터는 다시 캐시에 저장합니다.
- `log.info` / `log.warn` 호출은 데이터 출처(`dataSource`)와 캐시 TTL 정보를 남겨 운영 중 캐시 동작을 추적할 수 있게 합니다.
- DB 조회 중 예외가 발생했지만 캐시가 있다면 캐시 데이터를 반환해 서비스 가용성을 유지합니다.

### 12.3 일일 집계를 수행하는 배치 서비스 — `DailyStatsBatchService`
```java
// src/main/java/com/matjom/matjom/statistics/service/DailyStatsBatchService.java:38-78
public BatchResult runAggregation(LocalDate targetDate) {
    ...
    try {
        ...
        for (Map.Entry<Long, Aggregate> entry : aggregates.entrySet()) {
            ...
            batchRepository.upsertDailyStats(targetDate, placeId,
                    aggregate.starts, aggregate.arrives,
                    aggregate.reviews, aggregate.likes,
                    hourlyArrives, hourlyStarts,
                    peakHour, aggregatedAt);
...
```
- **설명**: 전일 방문/리뷰/좋아요 데이터를 조회해 `place_daily_stats` 테이블에 UPSERT하고, 시간대별 통계는 JSON 문자열로 변환하여 저장합니다. 처리 후 `place_stats` 캐시를 무효화합니다.
- `runLock`은 중복 실행을 방지하고, `lastStatus`에는 배치 상태를 기록해 API에서 조회할 수 있게 합니다.
- 마지막에 `DailyStatsPredictionService.scheduleRetraining`을 호출해 예측 연동 훅을 실행할 준비를 합니다.
...
- **`PlaceStatisticsQueryService`** (`src/main/java/com/matjom/matjom/statistics/service/PlaceStatisticsQueryService.java:16-41`)
...
## 13. 캐시 계층 역할 — `PlaceStatsCacheService`

```java
// src/main/java/com/matjom/matjom/statistics/cache/PlaceStatsCacheService.java:18-64
...
```
- `place:stats:{placeId}` 키로 Redis에 직렬화된 DTO를 저장해 통계 API의 Cache-aside 전략을 담당합니다.
- `get`은 캐시 JSON을 DTO로 다시 만들면서 데이터 출처를 `CACHE`로 표시하고, 역직렬화가 실패하면 키를 삭제해 손상된 캐시를 정리합니다.
- `put`은 서비스가 계산한 최신 통계를 TTL(기본 300초)과 함께 저장해, 같은 요청이 반복될 때 DB 부담과 응답 지연을 줄입니다.
- `evict`는 자정 배치나 운영 명령이 호출해 캐시를 비우는 용도로 사용합니다.
- 로그에 남는 `generatedAt`, `cacheTtlSeconds`, `StatsDataSource` 정보 덕분에 운영팀은 캐시 히트/미스를 쉽게 추적할 수 있습니다.
...
| `PlaceDetailFacadeService` | 통계 캐시(`PlaceStatisticsQueryService`)와 리뷰 서비스(`ReviewService`)를 호출하여 조합하고, 기본으로 최근 5개 리뷰만 반환하며 `reviewLimit` 파라미터로 확장/완전 조회를 조절합니다. |
| `PlaceDetailController` | `GET /api/places/{placeId}/detail` 엔드포인트로 사용자 화면에서 필요한 데이터를 한 번에 반환합니다. |
...
- **구조 이유**: 컨트롤러는 `placeId` 검증(`@Positive`)과 HTTP 응답 포맷만 담당하며, 캐시/DB 조회 흐름은 전부 `PlaceStatisticsQueryService`에 위임합니다. 이렇게 하면 컨트롤러 코드가 얇고 명확하게 유지됩니다.
...
- 캐시: `PlaceStatisticsQueryService` + `PlaceStatsCacheService` (Cache-aside, 기본 TTL 300초).
- 응답 필드: `placeName`, `totalVisitors`, `totalLikes`, `arrivals11To12`, `arrivals12To13`.
