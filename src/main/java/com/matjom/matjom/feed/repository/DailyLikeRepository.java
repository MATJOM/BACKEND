...
public interface DailyLikeRepository extends JpaRepository<DailyLike, UUID> {
    ...
    // 목적: 방문 ID로 좋아요 엔티티를 조회한다
    // 필요 이유: 서비스/테스트에서 저장된 좋아요를 식별하기 위함이다
    // 로직: visitId 조건을 만족하는 레코드를 Optional로 반환한다
    Optional<DailyLike> findByVisitId(Long visitId); // 9월 30일 최종: 자동 매칭 검증용
